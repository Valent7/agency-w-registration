...
def _content_format_from_assignment(assignment: str) -> str:
    """Определяет форму материала без отдельного запроса к ИИ."""
    lowered = str(assignment or "").lower().replace("ё", "е")
    if any(token in lowered for token in ("сценар", "ролик", "видео")):
        return "сценарий"
    if any(token in lowered for token in ("анонс", "приглашен", "приглашение")):
        return "анонс"
    if any(token in lowered for token in ("пост", "публикац")):
        return "пост"
    if any(token in lowered for token in ("сообщение команде", "текст для чата", "объявлен")):
        return "сообщение для команды"
    return "публикация"


def _w_story_editorial_standard() -> str:
    """Постоянный стандарт живого, но фактически честного контента Агентства W."""
    return r"""
W-STORY — РЕДАКЦИОННЫЙ СТАНДАРТ АГЕНТСТВА W

ЗАДАЧА
Писать не отчёт о функциях, а живой текст, который хочется дочитать.
Текст может ощущаться как маленький рассказ, новелла, сцена, наблюдение
или фрагмент хорошей прозы — но художественной является ФОРМА, а не факты.

ИЕРАРХИЯ ИСТОЧНИКОВ
1. Поручение Директора — источник конкретных фактов этого материала:
   тема, событие, дата, время, цитата, аудитория, цель, новый подтверждённый факт.
2. Фактический паспорт текущей версии Агентства W — источник возможностей системы.
3. Всё остальное запрещено выдавать за произошедшее или реально существующее.

ГЛАВНОЕ ПРАВИЛО ПРАВДЫ
НЕ ПРИДУМЫВАЙ ради красоты:
- время суток, даты, длительность, цифры и результаты;
- бытовые детали: кофе, ноутбук, прогулка, пробежка, кухня, офис и т. п.;
- реплики Директора, партнёров или агентов, если их не дал Директор;
- эмоции и мысли конкретного человека;
- встречи, ответы, кандидатов, согласия, отказы и другие события;
- новые функции Агентства.
Если красивой реальной сцены в источниках нет — НЕ СОЧИНЯЙ её.
Сделай крючком точное наблюдение, контраст, вопрос, парадокс или реальный факт.

ГИПОТЕТИЧЕСКАЯ СЦЕНА
Допустима только если она явно обозначена как модель ситуации:
«Представьте…», «Допустим…», «Знакомая ситуация: …».
Она не должна выглядеть как рассказ о событии, которое действительно произошло.
Не злоупотребляй этим приёмом.

ДРАМАТУРГИЯ W-STORY
Не превращай её в механический шаблон. Выбирай только нужные ступени:
1. КРЮЧОК — реальная деталь, реплика из поручения, наблюдение, вопрос или контраст.
2. ЧЕЛОВЕЧЕСКОЕ НАПРЯЖЕНИЕ — что в этой ситуации отнимает время, силы,
   уверенность, внимание или мешает получить результат.
3. ПОВОРОТ — что меняется благодаря конкретному действию Агентства W.
4. ДЕЙСТВИЕ — показать одну-две реальные функции через действие, а не перечень.
5. ЧЕЛОВЕЧЕСКИЙ СМЫСЛ — что остаётся человеку: решение, отношения, время,
   возможность сосредоточиться на главном.
6. ПОСЛЕВКУСИЕ — мысль, образ, лёгкая интрига или точный вопрос.

ИНТРИГА
Интрига должна рождаться из смысла и последовательности раскрытия фактов.
Не создавай искусственную тайну и не прячь главное слишком долго.
Не используй дешёвые клише: «И вот здесь начинается самое интересное»,
«всё изменилось в один момент», «будущее уже наступило», если без них текст сильнее.

ЯЗЫК
- живой русский язык, который приятно читать вслух;
- конкретные глаголы и существительные вместо канцелярита;
- эмоциональность без пафоса и крика;
- литературность без вычурности;
- одна сильная мысль лучше пяти рекламных тезисов;
- короткие и длинные фразы можно чередовать ради ритма;
- избегай маркетингового жаргона и технического отчёта;
- не объясняй очевидное после сильной фразы;
- не повторяй одни и те же фирменные обороты из публикации в публикацию.

НЕ ПИСАТЬ ТАК
«Сегодня Агентство внедрило…» + список функций.
«Стагирит умеет 1…, 2…, 3…».
Фальшивые мини-истории с придуманным утром, чашкой кофе, календарным окном,
пробежкой, улыбкой, офисом или диалогом, которых не было в исходных данных.

ПИСАТЬ ТАК
Сначала найти, ЧТО В ЭТОМ ФАКТЕ ВАЖНО ДЛЯ ЧЕЛОВЕКА.
Затем выбрать художественную форму, которая раскрывает именно этот смысл.
Функцию показывать только там, где она двигает историю или мысль.

ФОРМАТЫ
ПОСТ:
- цельный авторский текст;
- по возможности без списков и служебных подзаголовков;
- может быть мини-новеллой, наблюдением, диалогом или эссе;
- финал не обязан каждый раз заканчиваться девизом или призывом.

АНОНС:
- быстрее поста;
- крючок → почему это важно → что человек увидит/поймёт → точные организационные данные;
- дата, время, ссылка и тема только из поручения или сохранённого контекста;
- приглашение естественное, без рекламного нажима.

СЦЕНАРИЙ:
- можно использовать сцены, кадры, голос за кадром, диалог;
- если персонаж и ситуация придуманы для ролика, это должен быть явно собирательный
  художественный образ, а не «реальный случай Валентины»;
- визуальное действие должно показывать смысл, а не интерфейс ради интерфейса.

СООБЩЕНИЕ ДЛЯ КОМАНДЫ:
- короче и прямее;
- сохраняет живой голос и интригу, но не превращается в рассказ любой ценой.

КРИТЕРИЙ КАЧЕСТВА
После чтения человек должен:
- понять одну главную мысль;
- почувствовать, зачем это ему;
- не заметить технического отчёта;
- не поймать автора на выдуманной «красивой» детали;
- захотеть дочитать до конца.
""".strip()
...
def _generate_content(
    ...
    """
    Редакционный цикл W-STORY:
    1) Стагирит-редактор выделяет правду, смысл и драматургию.
    2) Мастер контента пишет в выбранном жанре.
    3) Стагирит-главред проводит факт-аудит и литературную редактуру.
    Пользователь видит только финальный материал.
    """
    ...
    zoom_context = (
        ...
        if zoom_link
        else "Ссылка Zoom пока не сохранена."
    )
    facts = _agency_current_release_brief()
    standard = _w_story_editorial_standard()
    content_format = _content_format_from_assignment(assignment)

    # ШАГ 1. Стагирит не сочиняет сюжет, а сначала отделяет факты от литературной формы.
    brief_prompt = f"""
Ты — Стагирит, заместитель Директора и РЕДАКЦИОННЫЙ ДИРЕКТОР Агентства W.

Директор: {owner_name}.
Формат материала: {content_format}.
Поручение Директора:
{assignment}

ФАКТИЧЕСКИЙ ПАСПОРТ ТЕКУЩЕЙ ВЕРСИИ:
{facts}
...
Не пиши финальный текст. Подготовь короткий внутренний бриф Мастеру контента.

СНАЧАЛА СДЕЛАЙ ФАКТ-АУДИТ:
- перечисли 3–8 конкретных фактов, которые РАЗРЕШЕНО использовать;
- отдельно перечисли то, что НЕЛЬЗЯ додумывать в этом материале;
- если в поручении нет реальной сцены, прямо напиши: «реальную сцену не выдумывать»;
- если есть точная реплика/событие от Директора, можно строить вокруг него сюжет.

ЗАТЕМ ОПРЕДЕЛИ:
- кто читатель и зачем ему дочитывать;
- одна главная человеческая мысль;
- лучший крючок, основанный на правде;
- жанр и степень литературности;
- какое напряжение или вопрос ведёт текст;
- 1–3 реальных действия/детали, через которые раскрыть смысл;
- что раскрыть не сразу, чтобы сохранить естественное любопытство;
- финальное послевкусие;
- нужен ли CTA. Если Директор не просил звать/продавать — CTA не нужен.

Запрет: не предлагай придуманные утро, кофе, ноутбук, прогулку, календарное окно,
реплики Валентины или других людей, если их нет в исходных данных.

Верни только профессиональный внутренний бриф.
""".strip()
    ...
    # ШАГ 2. Мастер пишет художественно, но только внутри подтверждённых фактов.
    master_prompt = f"""
...
РЕДАКЦИОННЫЙ СТАНДАРТ W-STORY:
{standard}

НАПИШИ ТОЛЬКО ГОТОВЫЙ МАТЕРИАЛ.

Ключевое требование:
сделай текст интересным НЕ за счёт выдуманных событий, а за счёт точного наблюдения,
ритма, хорошей драматургии, человеческого смысла и последовательного раскрытия фактов.

Жёстко:
- ни одной конкретной бытовой детали, реплики, цифры, результата или события,
  которого нет в поручении/фактическом паспорте;
- не выдавай гипотетический пример за реальный случай;
- не перечисляй функции подряд;
- не начинай с технического описания Агентства;
- не используй фальшивую сенсационность;
- не вставляй Zoom, ссылку, продажу или CTA без основания;
- соблюдай заданный Директором лимит длины, если он есть.

Если реального сюжета недостаточно для мини-новеллы, выбери сильное наблюдение,
контраст, вопрос или эссеистическую форму. Не насилуй материал «рассказом».

Только финальный текст. Без пояснений редактора.
""".strip()
    ...
    # ШАГ 3. Главред в первую очередь проверяет правду, затем литературное качество.
    chief_prompt = f"""
Ты — Стагирит, ГЛАВНЫЙ РЕДАКТОР Агентства W.
...
ПРОВЕДИ МОЛЧА ДВА АУДИТА.

АУДИТ 1 — ПРАВДА
Для КАЖДОЙ конкретной детали мысленно спроси: откуда она взялась?
Допустимы только:
A) прямой факт из поручения Директора;
B) факт из паспорта текущей версии;
C) явно обозначенная гипотетическая ситуация, если она действительно нужна жанру.
Если источник не находится — УДАЛИ или ПЕРЕПИШИ деталь.
Особенно проверяй: время суток, кофе, ноутбук, офис, прогулки, эмоции,
реплики, цифры, встречи, ответы людей, результаты и действия агентов.

АУДИТ 2 — ЛИТЕРАТУРНОЕ КАЧЕСТВО
1. После первых строк хочется продолжить?
2. Есть одна сильная человеческая мысль?
3. Есть движение текста, а не перечень функций?
4. Интрига естественная, а не искусственная?
5. Текст эмоционален без пафоса?
6. Нет канцелярита, рекламных штампов и «красивостей ради красивостей»?
7. Функции показаны только там, где они раскрывают смысл?
8. Формат соблюдён: пост/анонс/сценарий/сообщение?
9. Финал оставляет мысль, образ или желание узнать продолжение?
10. Текст звучит как авторский, а не как типовой ответ ИИ?

Если что-то существенно слабое — перепиши весь нужный фрагмент.
Не делай текст суше только ради безопасности: сохрани ритм, характер и эмоцию.

Ориентир качества: 9/10 и выше.
В ответе — ТОЛЬКО финальный материал. Никаких оценок и комментариев.
""".strip()

    final_text = ask_openai_fn(
        chief_prompt,
        draft,
        uploaded_files=[],
        use_web_search=False,
...
def _make_plan(intents: list[str], meeting_count: int) -> list[str]:
    ...
    if "content" in intents:
        plan.extend([
            "Сформировать материал через редакционный цикл W-STORY: факты → бриф → текст → проверка.",
...
def _render_result(
    ...
    if result.get("content"):
        ...
        st.caption(
            "Стагирит поставил творческую задачу, Мастер написал, "
            "а Стагирит-главред проверил текст перед показом вам. "
            "Дальше — ваша правка и утверждение."
        )
