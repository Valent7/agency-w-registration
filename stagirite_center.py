...
def _generate_content(
    ...
    """
    Три реальные роли, а не один длинный промт:
    1) Стагирит-редактор создаёт творческий бриф.
    2) Мастер контента пишет.
    3) Стагирит-главред проверяет и при необходимости переписывает.
    Пользователь видит только финал.
    """
    ...
    # ШАГ 1. Стагирит превращает простую тему в редакционный бриф.
    brief_prompt = f"""
Ты — Стагирит, мудрый заместитель Директора Агентства W.
Сейчас ты работаешь как сильный РЕДАКЦИОННЫЙ ДИРЕКТОР.
...
Не пиши сам пост. Составь внутренний творческий бриф Мастеру контента.

Определи:
- кто читатель;
- зачем ему дочитывать;
- какая эмоция нужна;
- какая интрига/крючок;
- какой жанр лучше: история, сцена, диалог, наблюдение, притча,
  контраст, ироничный мини-рассказ или анонс;
- одна главная мысль;
- 2–4 конкретные сцены/детали;
- где уместен мягкий юмор;
- что оставить недосказанным;
- какой финальный образ или фраза должны остаться в памяти;
- нужен ли CTA. Если Директор не просил продавать/звать — CTA НЕ нужен.

Главное:
«Люди читают не функции. Люди читают жизнь, в которой узнают себя».

Если тема позволяет историю, пост должен ощущаться как маленький фильм.
Функции Агентства показывай через действия героев, а не через перечень.

Верни только короткий профессиональный бриф.
""".strip()
    ...
    # ШАГ 2. Мастер пишет по реальному брифу.
    master_prompt = f"""
Ты — Мастер контента Агентства W.
Ты получил редакционный бриф от Стагирита.
...
НАПИШИ ТОЛЬКО ГОТОВУЮ ПУБЛИКАЦИЮ.

Твоя планка:
- начать из любопытства;
- продолжить из удовольствия;
- закончить с мыслью или улыбкой.

Жёсткие правила:
- не писать отчёт;
- не начинать с описания функций;
- первые два абзаца — сцена/деталь/реплика/интрига, если жанр это допускает;
- функции показывать через действие;
- бытовая конкретика лучше абстрактной пользы;
- юмор добрый, наблюдательный, не клоунада;
- не использовать вычурные метафоры, которые приходится расшифровывать;
- не вставлять Zoom/«приходите»/продажу без прямой просьбы;
- не выдумывать возможности;
- если Директор задал лимит длины — соблюдать;
- если лимита нет, история может быть длинной, пока каждый абзац работает.

Персонажи:
Стагирит — взрослый мужчина-координатор.
...
Только публикация. Никаких комментариев к ней.
""".strip()

    # Claude — основное «перо» Мастера контента.
    # Если ключ ещё не сохранён/невалиден/сервис временно недоступен,
    # Агентство не ломается: Стагирит использует резервный OpenAI.
    claude_result = None
    ...
    if (
        ...
        draft = str(claude_result["text"]).strip()
        st.session_state[
            f"stagirite_content_master_engine_{owner_id}"
        ] = "primary"
        st.session_state.pop(
            f"stagirite_content_master_error_{owner_id}",
            None,
        )
    else:
        draft = ask_openai_fn(
    ...
    # ШАГ 3. Стагирит-главред читает реальный текст и отвечает за качество.
    chief_prompt = f"""
Ты — Стагирит, теперь ГЛАВНЫЙ РЕДАКТОР Агентства W.
Перед публикацией ты обязан проверить текст Мастера контента.
...
МОЛЧА оцени:
1. Крючок: хочется ли читать после первых 2–3 строк?
2. Живость: видит ли читатель сцену, людей и действие?
3. Интрига: есть ли движение/недосказанность?
4. Юмор: есть ли уместная человеческая улыбка, если тема позволяет?
5. Польза: показана ли через жизнь, а не презентационные тезисы?
6. Естественность: нет ли искусственных метафор типа «уборщик тишины»?
7. Правда: нет ли выдуманных/устаревших функций?
8. CTA: не продаёт ли текст то, чего Директор не просил?
9. Финал: остаётся ли после него мысль или фраза?
10. Ритм: нет ли абзацев, похожих на отчёт.

Если хотя бы один ключевой пункт слабый — ПЕРЕПИШИ.
Сохраняй сильные находки, но не защищай первоначальный текст.

Ориентир качества: 9/10.
Пользователь должен получить не «нормальный текст», а публикацию,
которую хочется дочитать и переслать.

В ответе — ТОЛЬКО финальная публикация.
Никаких оценок, пояснений и замечаний.
""".strip()
...
def _render_result(
    ...
    if result.get("content"):
        ...
        master_engine = st.session_state.get(
            f"stagirite_content_master_engine_{owner_id}"
        )
        if master_engine == "primary":
            st.caption("🟢 Мастер контента: основной литературный движок подключён.")
        elif master_engine == "reserve":
            st.caption(
                "🟡 Мастер контента: временно работает в резервном режиме. "
