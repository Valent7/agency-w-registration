...
def _generate_content(
    ...
    """
    W-STORY production line:
    1. Стагирит создаёт режиссёрский/редакционный бриф.
    2. Мастер контента пишет живую историю.
    3. Стагирит НЕ переписывает её сам — только оценивает.
    4. Если слабая — Мастер переписывает по замечаниям Стагирита.
    Финальный художественный текст всегда выходит из-под пера Мастера.
    """
    ...
    zoom_context = (
        f"Сохранённая Zoom-ссылка: {zoom_link}\n"
        f"Дополнение: {zoom_note or 'нет'}"
        if zoom_link
        else "Zoom-ссылка не сохранена."
    )

    # ------------------------------------------------------------------
    # ШАГ 1. Стагирит = редакционный режиссёр.
    # ------------------------------------------------------------------
    brief_prompt = f"""
Ты — Стагирит, мудрый заместитель Директора Агентства W.
Сейчас твоя роль — РЕДАКЦИОННЫЙ РЕЖИССЁР.
...
Твоя задача — поставить Мастеру контента такую творческую задачу,
чтобы из простых слов Директора родился текст, который хочется читать.

ГЛАВНАЯ СИСТЕМА АГЕНТСТВА W — W-STORY:

1. ЧЕЛОВЕК И ЖИВАЯ СЦЕНА.
   Сначала человек, время, место, бытовая деталь, реплика или действие.
   Не начинай с продукта.

2. НАПРЯЖЕНИЕ / БОЛЬ.
   Покажи маленькую узнаваемую проблему: рутина, нехватка времени,
   повторяющиеся действия, страх что-то упустить.

3. ВОПРОС / ИНТРИГА.
   Дай причину читать дальше. Не раскрывай ответ сразу.

4. ИИ-КОМАНДА В ДЕЙСТВИИ.
   Стагирит, Неония, Неона, Неола, Мастер, Художник появляются
   только там, где нужны сюжету. Не перечисляй их должностные инструкции.
   Показывай действие.

5. ПРЕВРАЩЕНИЕ / РЕЗУЛЬТАТ.
   Покажи, что изменилось в дне человека благодаря Агентству.

6. ЧЕЛОВЕЧЕСКИЙ СМЫСЛ.
   Время, отношения, выбор, спокойствие, творчество, жизнь.

7. ФИНАЛ.
   Одна короткая мысль, улыбка, поворот или любопытство:
   «а что дальше?». Не подводи итог как в отчёте.

Стагирит должен САМ выбрать форму:
мини-новелла, сцена, диалог, наблюдение, ироничный рассказ,
контраст, маленькая притча или эмоциональный анонс.

ВАЖНО:
- для обычного поста НЕТ автоматического CTA;
- Zoom/«приходите» только если Директор прямо просит анонс/приглашение;
- используй максимум 2–3 факта о функциях, необходимые сюжету;
- технические факты сначала переводи в человеческое действие;
- мягкий юмор должен возникать из наблюдения, а не быть приклеенной шуткой;
- никакого канцелярита;
- никаких вычурных метафор, которые надо разгадывать;
- никаких выдуманных функций.

Верни только творческий бриф для Мастера.
Не пиши сам пост.
""".strip()
    ...
    # ------------------------------------------------------------------
    # Writer helper: Claude primary, OpenAI reserve.
    # ------------------------------------------------------------------
    def write_with_master(system_prompt: str, user_text: str) -> tuple[str, dict[str, Any]]:
        claude_result = None
        ...
        if isinstance(claude_result, dict):
            error_text = str(
                ...
                or claude_result.get("error_code")
                or ""
            ).strip()
        return (
...
Стагирит поставил тебе редакционную задачу.
...
ТВОЯ ПЛАНКА:
читатель начинает из любопытства,
продолжает из удовольствия,
заканчивает с мыслью или улыбкой.

ПИШИ КАК МАЛЕНЬКУЮ ИСТОРИЮ, ЕСЛИ ЖАНР ПОЗВОЛЯЕТ:
- сцена важнее объяснения;
- конкретная деталь важнее слова «эффективность»;
- действие важнее перечня функций;
- один живой человек важнее десяти тезисов;
- диалог или наблюдение лучше рекламного лозунга;
- лёгкая ирония приветствуется;
- абзацы короткие и визуальные.

КАТЕГОРИЧЕСКИ НЕЛЬЗЯ В ОБЫЧНОМ ПОСТЕ:
«Представьте модельный рабочий день»,
«недельная цель ставится»,
«система ролей и правил»,
«резерв держится»,
«функционал системы»,
длинные объяснения алгоритмов,
перечень функций одного агента за другим.

Не объясняй архитектуру. ПОКАЖИ ЕЁ В ЖИЗНИ.

Если тема — «один день Агентства W»:
читатель должен словно увидеть два параллельных мира:
Агентство работает, а человек получает обратно свою жизнь.
Но придумай собственные сцены — не копируй один и тот же сюжет механически.

Персонажи, если они нужны:
...
- Неола — взрослая женщина-наставник взрослого новичка;
- Директор — живой взрослый человек.

Только готовая публикация.
Без анализа, без заголовков «структура поста», без комментариев.
""".strip()
    ...
    # ------------------------------------------------------------------
    # ШАГ 3. Стагирит = критик, но НЕ автор.
    # ------------------------------------------------------------------
    critic_prompt = f"""
Ты — Стагирит, главный редактор Агентства W.
Твоя задача — не переписывать пост.
Ты оцениваешь работу Мастера и даёшь точное редакторское решение.
...
hook — первая строка;
scene — видно ли сцену;
intrigue — хочется ли читать дальше;
humanity — в центре ли человек;
humor — есть ли уместная улыбка;
rhythm — нет ли отчётных провалов;
ending — запоминается ли финал;
truth — фактическая точность.

FATAL = true, если:
- текст похож на отчёт/презентацию;
- перечисляет функции вместо истории;
- придумал функцию;
- обычный пост самовольно зовёт на Zoom/что-то продаёт;
- первая половина почти полностью состоит из объяснений системы.
...
{{
  ...
  "strength": "одно короткое предложение",
  "revision_brief": "конкретные указания Мастеру, как сделать текст 9/10"
}}

PASS только если текст действительно хочется дочитать.
...
""".strip()
    ...
    # ------------------------------------------------------------------
    # ШАГ 4. Если плохо — замечания идут ОБРАТНО МАСТЕРУ.
    # Стагирит сам не переписывает художественный текст.
    # ------------------------------------------------------------------
    if force_rewrite:
        revision_brief = str(
            review.get("revision_brief")
            or (
                "Текст слишком похож на объяснение системы. "
                "Перепиши как живую мини-историю по W-STORY: "
                "человек и сцена → напряжение → интрига → команда в действии → "
                "изменение → человеческий смысл → сильный короткий финал. "
                "Убери технические списки и канцелярит."
            )
        ).strip()

        rewrite_prompt = f"""
Ты — Мастер контента Агентства W.
Стагирит НЕ принял первую версию и вернул её тебе на редактуру.
...
Перепиши полностью там, где это нужно.
Не косметическая правка — сделай живой текст уровня 9/10.

Обязательный W-STORY:
человек/сцена → напряжение → интрига → ИИ-команда в действии →
изменение → человеческий смысл → сильный финал.

Особенно:
- первые 3 строки должны открыть сцену, а не систему;
- не употребляй технические факты как абзацы отчёта;
- если факт нельзя естественно показать действием — лучше не используй его;
- добавь наблюдательный юмор, если тема позволяет;
- никакого CTA без просьбы Директора.

Только финальная публикация.
""".strip()
        ...
        if rewritten:
            ...
            rewrite_used = True
            # Если первая попытка шла в резерве, а повтор уже прошёл через основной
            # движок — сохраняем реальный финальный движок.
            if rewrite_meta.get("engine") == "primary":
                engine_meta = rewrite_meta

    # ------------------------------------------------------------------
    # ШАГ 5. Локальная страховка: не отдаём явно отчётный финал молча.
    # Ещё один AI-вызов не делаем — помечаем качество для интерфейса.
    # ------------------------------------------------------------------
    final_report_like = _looks_like_report(final_text)

    return {
        "text": final_text,
        "master_engine": str(engine_meta.get("engine") or "reserve"),
        "master_model": str(engine_meta.get("model") or ""),
        "master_error": str(engine_meta.get("error") or ""),
        "quality_score": int(score),
        "rewrite_used": bool(rewrite_used),
        "report_warning": bool(final_report_like),
    }
...
def _make_plan(intents: list[str], meeting_count: int) -> list[str]:
    ...
    if "content" in intents:
        plan.extend([
            "Стагириту сформировать творческий бриф.",
            "Мастеру контента подготовить живой текст по брифу.",
            "Стагириту-главреду проверить текст перед показом Директору.",
            "Если нужна иллюстрация — поставить задачу Художнику.",
        ])
...
def _render_result(
    ...
    if result.get("content"):
        ...
        master_engine = str(master_meta.get("engine") or "")
        quality_score = int(master_meta.get("quality_score") or 0)

        if master_engine == "primary":
            st.caption(
                "🟢 Мастер контента подключён и написал эту публикацию."
            )
        elif master_engine == "reserve":
            st.caption(
                "🟡 Мастер контента сейчас работает через резервный движок. "
                "Публикация не остановлена."
            )

        if master_meta.get("rewrite_used"):
            st.caption(
                "🧐 Стагирит вернул первую версию Мастеру на доработку "
                "до показа вам."
            )

        if quality_score:
            st.caption(
                f"Редакторская оценка Стагирита: {quality_score}/10."
            )

        if master_meta.get("report_warning"):
            st.warning(
                "Стагирит видит остаточные признаки отчётного стиля. "
                "Этот текст лучше не утверждать без дополнительной правки."
            )
        ...
        st.caption(
            "Стандарт W-STORY: живая сцена → напряжение → интрига → "
            "Агентство в действии → изменение → человеческий смысл → финал."
        )
