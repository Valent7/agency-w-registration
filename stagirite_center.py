...
def _agency_current_release_brief() -> str:
    ...
    return """
ФАКТЫ О ТЕКУЩЕЙ ВЕРСИИ АГЕНТСТВА W (21.09.2026):
...
- В Агентстве работает раздел «Диалоги Telegram»: он показывает людей, с которыми уже начата работа,
  и визуально обозначает состояние разговора. Жёлтый статус — ждём ответа, зелёный — диалог идёт,
  фиолетовый — идёт согласование встречи.
- В Агентстве работает раздел «Диалоги VK»: начатые ветки сохраняются отдельно от ежедневной
  подборки Неонии, поэтому человек не исчезает из поля зрения только потому, что сегодня не попал
  в новую подборку.
- После согласования встречи её дата, время и формат отображаются в календаре Агентства.
- Разделы диалогов нужны прежде всего для экономии внимания и времени владельца: ему не нужно
  вручную вспоминать, кому он уже писал, и заново разыскивать каждый разговор в ленте.
...
def _content_mode_from_assignment(assignment: str) -> str:
    """Выбирает форму текста по реальному намерению Директора.

    Раньше любой обычный «пост» по умолчанию проваливался в режим story,
    поэтому Мастер мог сочинить семью, бытовую сцену и длинный рассказ там,
    где Директор просил просто и наглядно показать пользу инструмента.
    """
    ...
    # Художественный режим включаем только по явной просьбе.
    if any(
        token in lowered
        for token in (
            "хроник",
    ...
    # Обычный пост/публикация/объяснение — не художественная проза.
    if any(
        token in lowered
        for token in (
            "пост",
    ...
    # Без явного запроса на художественный жанр выбираем безопасный
    # короткий прямой текст, а не выдуманную историю.
    return "direct_post"
...
def _direct_post_is_good(text: str, assignment: str) -> bool:
    """Минимальная страховка от самовольного ухода в художественный рассказ."""
    value = str(text or "").strip()
    source = str(assignment or "").lower().replace("ё", "е")
    lowered = value.lower().replace("ё", "е")
    ...
    short_requested = any(
        token in source
        for token in ("небольш", "коротк", "кратк", "до 1000", "до 1200")
    )
    if short_requested and len(value) > 1500:
        return False
    if len(value) > 2600:
        return False

    # Не сочиняем семью, кухню и других героев, если Директор их не давал.
    invented_story_markers = (
        "жена ",
        "муж ",
        "дочка",
        "сын ",
    ...
    for marker in invented_story_markers:
        if marker in lowered and marker not in source:
            return False

    # Не подмешиваем ежедневную пятёрку в пост о другом инструменте.
    if "пятерк" in lowered and "пятерк" not in source:
        return False

    # Если Директор назвал оба раздела, оба должны остаться в тексте.
    if "диалоги telegram" in source and "диалоги telegram" not in lowered:
        return False
    if "диалоги vk" in source and "диалоги vk" not in lowered:
        return False

    # Если смысл задания держится на цветовых статусах — не теряем их.
    for stem in ("желт", "зелен", "фиолет"):
        if stem in source and stem not in lowered:
            return False

    return True


def _generate_direct_post_content(
    ask_openai_fn,
    ask_claude_fn,
    assignment: str,
) -> dict[str, Any]:
    """Короткий прямой пост о пользе/инструменте без выдуманной истории."""
...
Это НЕ «Хроники» и НЕ литературный рассказ.
Твоя задача — сделать короткий, ясный, живой пост, который быстро показывает
ОДНУ конкретную пользу инструмента или функции.

ГЛАВНОЕ ПРАВИЛО:
Сначала пойми, что именно Директор хочет показать читателю.
Не заменяй его задачу своей красивой идеей.

ИЕРАРХИЯ ИСТОЧНИКОВ:
1. Факты и формулировки, которые Директор дал в поручении, — главное.
2. Банк правды ниже нужен только для проверки и минимального контекста.
3. НЕ добавляй другие возможности Агентства только потому, что они существуют.
4. Если факт не дан Директором и не подтверждён Банком правды — не придумывай.

БАНК ПРАВДЫ:
{facts}

КАК ПИСАТЬ:
- по умолчанию 500–1200 знаков; если Директор задал длину — соблюдай её;
- если сказано «небольшой/короткий» — стремись к 500–900 знакам;
- один связный текст, без списка, если список прямо не просили;
- первые 1–2 строки должны сразу назвать знакомую проблему или выгоду;
- затем очень просто показать, как инструмент решает её;
- использовать реальные названия разделов, кнопок, статусов и цветов из поручения;
- закончить конкретным человеческим результатом: сэкономленное время, меньше рутины,
  ясность, освобождённое внимание;
- можно лёгкую естественную фразу, но без рекламного пафоса.

КАТЕГОРИЧЕСКИ НЕЛЬЗЯ БЕЗ ПРЯМОЙ ПРОСЬБЫ:
- выдумывать жену, мужа, детей, семью, ужин, кухню, поездку или другого героя;
- сочинять диалоги и бытовые сцены;
- превращать текст в рассказ, новеллу или фельетон;
- добавлять «ежедневную пятёрку», поиск кандидатов или другие функции,
  если пост не о них;
- объявлять уже работающую функцию «ещё не запущенной»;
- придумывать числа, сроки, результаты и гарантии;
- объяснять архитектуру ИИ и техническую кухню;
- писать длинное вступление до сути.

ЕСЛИ ПОСТ О РАЗДЕЛЕ ИНТЕРФЕЙСА:
покажи путь «раньше было неудобно → теперь один взгляд → что означают статусы →
куда переходит согласованная встреча → сколько времени/внимания это экономит»,
но только если эти детали действительно есть в поручении.

Верни ТОЛЬКО готовый текст поста. Без комментариев редактора.
""".strip()
    ...
    if (
        final_text
        and not _direct_post_is_good(final_text, assignment)
        and callable(ask_claude_fn)
    ):
        retry_prompt = f"""
Перепиши текст строго по исходному поручению Директора.
...
ИСПРАВЬ:
- никаких выдуманных героев, семьи, ужина и бытовой сцены;
- никаких функций, которых не просил Директор;
- не подмешивай ежедневную пятёрку, если речь не о ней;
- не называй работающую функцию незапущенной;
- покажи конкретный инструмент и его простоту;
- сохрани названия разделов, статусы, цвета, время и цифры из поручения;
- один короткий связный пост, без списка, если список не просили;
- суть должна быть понятна в первых строках;
- финал — конкретная польза человеку.
...
""".strip()
        try:
            ...
            if (
                ...
                final_text = str(retry.get("text") or "").strip()
                engine = "primary"
                model = str(retry.get("model") or model)
                rewrite_used = True
        except Exception:
    ...
    if not final_text:
        final_text = (
            "Хороший инструмент экономит не только действия — он освобождает внимание. "
            "В Агентстве W нужное состояние работы видно сразу в одном месте, поэтому "
            "не приходится снова искать переписки и держать всё в голове. "
            "Меньше рутины — больше времени на сам разговор и на человека."
        )
        engine = "deterministic_fallback"
    ...
    return {
        ...
        "report_warning": not _direct_post_is_good(final_text, assignment),
        "series": "",
        "genre": "короткий прямой пост",
        "capability": "",
        "human_situation": "",
        "human_gain": "",
        "reader_entry": "",
...
def _generate_content(
    ...
    # Обычный пост/инструкция/объяснение пользы — отдельный прямой контур.
    # Здесь запрещаем самовольный storytelling.
    if mode in {"direct_post", "informational"}:
        return _generate_direct_post_content(
            ask_openai_fn,
            ask_claude_fn,
            assignment,
        )
