...
# Жёсткая граница правды для Неоны. Эти возможности подтверждены текущей системой.
_AGENCY_W_CAPABILITY_BOUNDARY = """
СЕГОДНЯ МОЖНО УТВЕРЖДАТЬ ТОЛЬКО ТО, ЧТО УЖЕ РЕАЛЬНО ЕСТЬ В АГЕНТСТВЕ W:
- Агентство W — виртуальная команда ИИ-помощников, которая снимает часть рутины и возвращает владельцу время;
- Неония работает с подключёнными Telegram-контактами и чатами: помогает искать и анализировать подходящих людей;
- Неона поддерживает входящие диалоги в подключённых каналах, отвечает по контексту и может мягко довести заинтересованного человека до встречи с владельцем аккаунта;
- система помогает готовить персональные первые сообщения, а важные решения и утверждение остаются за человеком;
- календарь используется для согласования реальной встречи, когда человек к ней готов.

НЕ ПРИПИСЫВАЙ АГЕНТСТВУ W ТО, ЧЕГО НЕТ ИЛИ ЧТО НЕ ПОДТВЕРЖДЕНО:
- собственную внешнюю базу лидов, поиск по профессиональным площадкам как готовую услугу;
- платную рекламу, закупку трафика или рекламные кампании;
- CRM как продукт и готовые интеграции с CRM;
- скрининг/интервью кандидатов как отдельную оказываемую услугу;
- готовые тарифы, цены, сметы, пакеты «базовый/оптимальный/полный»;
- коммерческие предложения, которые ты якобы отправишь по e-mail;
- 24/7 обслуживание, ежемесячную отчётность и другие сервисные обязательства, если они явно не подтверждены системой.
Если человек спрашивает о такой функции, спокойно скажи, что не будешь обещать того, чего сейчас нет, и объясни ближайшую реально работающую пользу.
Если уже понятна личная задача человека, это хороший момент предложить не «прайс» и не «пакет», а короткую встречу с владельцем аккаунта, чтобы показать работающую систему на его примере.
""".strip()


_UNAPPROVED_OFFER_PATTERNS = (
    r"\b(?:(?:могу|можем)\s+)?(?:отправить|отправлю|прислать|пришлю|подготовить|подготовлю|составить|составлю|выслать|вышлю)\b[^.!?\n]{0,160}\b(?:коммерческ\w*\s+предложен\w*|смет\w*|расцен\w*|прайс\w*|тариф\w*|пакет\w*)\b",
    r"\b(?:отправлю|пришлю|вышлю|могу\s+отправить|могу\s+прислать)\b[^.!?\n]{0,120}\b(?:e-?mail|email|почт\w*)\b",
    r"\b(?:базов\w*|оптимальн\w*|полн\w*)\s+(?:пакет|тариф)\w*\b",
    r"\b(?:добавить|обеспечить|предоставить|включить)\s+24\s*/\s*7\b",
    r"\bмы\s+(?:запускаем|настраиваем|вед[её]м)\s+платн\w*\s+реклам\w*\b",
    r"\bмы\s+(?:можем\s+)?(?:интегрировать|интегрируем|подключаем)\w*\s+(?:с\s+)?crm\b",
    r"\bиз\s+собственной\s+базы\b",
    r"\bчерез\s+профессиональн\w*\s+площадк\w*\b",
    r"\bмы\s+проводим\s+(?:скрининг|интервью)\b",
    r"\bквалификац\w*\s+(?:лидов|кандидат\w*)\b",
    r"\bпередач\w*\s+(?:лидов\s+)?менеджер\w*\b",
    r"\bежемесячн\w*\s+отч[её]т\w*\b",
)


def _contains_unapproved_offer(value: str) -> bool:
    normalized = re.sub(r"\s+", " ", str(value or "").casefold()).strip()
    if not normalized:
        return False

    if any(re.search(pattern, normalized, flags=re.IGNORECASE) for pattern in _UNAPPROVED_OFFER_PATTERNS):
        return True

    # Упоминать платную рекламу можно только как НЕподдерживаемую функцию.
    # Нейтральное/положительное описание рекламы («обычно даёт быстрый приток…»)
    # превращает Неону в выдуманного рекламного подрядчика и должно быть остановлено.
    for match in re.finditer(r"платн\w*\s+реклам\w*", normalized):
        before = normalized[max(0, match.start() - 70):match.start()]
        after = normalized[match.end():match.end() + 90]
        negated = any(token in before for token in ("не ", "не обещ", "не запуска", "нет ")) or any(
            token in after for token in ("не обещ", "не запуска", "не является", "не подтвержд")
        )
        if not negated:
            return True
...
def _capability_guard_fallback(owner_name: str, incoming_text: str, context) -> str:
    """Детерминированный безопасный ответ, если модель снова придумала услугу."""
    forms = _owner_forms(owner_name)
    lowered = re.sub(r"\s+", " ", str(incoming_text or "").casefold()).strip()

    if any(token in lowered for token in ("цена", "стоим", "расцен", "прайс", "тариф", "пакет", "смет", "коммерческ")):
        return (
            "Я не буду придумывать цены или пакеты — утверждённых расценок у меня сейчас нет. "
            "Могу точно рассказать и показать то, что уже работает в Агентстве W; "
            f"если это подходит под вашу задачу, познакомлю вас с {forms['instrumental']}, чтобы посмотреть систему на вашем примере."
        )

    if any(token in lowered for token in ("реклам", "трафик", "лиды")):
        return (
            "Платную рекламу и закупку трафика я обещать не буду — это не подтверждённая функция Агентства W. "
            "Зато система уже помогает работать с подходящими контактами, диалогами и встречами, снимая часть рутины. "
            f"Если для вас сейчас важен поток новых людей, можно показать с {forms['instrumental']}, где именно W реально сможет помочь в вашем процессе."
        )

    if "crm" in lowered:
        return (
            "Не совсем: Агентство W — не CRM. CRM в основном хранит и упорядочивает данные, "
            "а W — это команда ИИ-помощников, которая помогает выполнять часть реальной работы; "
            "готовую интеграцию с CRM я сейчас обещать не буду."
        )

    if "@" in lowered or "почт" in lowered or "email" in lowered or "e-mail" in lowered:
        return (
            "Спасибо, адрес вижу. Но я не буду обещать отправку коммерческого предложения по почте — такой автоматической отправки у меня сейчас нет. "
            f"Если ваша задача уже понятна, лучше я помогу организовать короткую встречу с {forms['instrumental']}, где можно показать работающие возможности Агентства W на вашем примере."
        )

    return (
        "Чтобы не обещать лишнего: я могу говорить только о тех возможностях Агентства W, которые уже реально работают. "
        "Это помощь с поиском и анализом подходящих контактов, диалогами, подготовкой персональных сообщений и организацией встреч — при сохранении контроля у человека."
    )


def _guard_reply_against_unapproved_offers(config, owner_name: str, incoming_text: str, reply: str, context) -> str:
    if not _contains_unapproved_offer(reply):
        return reply

    history = _dialog_context_block(context, max_turns=6)
    forms = _owner_forms(owner_name)
    instructions = f"""
Ты редактируешь ответ Неоны перед отправкой человеку.
В исходном ответе появились НЕПОДТВЕРЖДЁННЫЕ услуги или обещания. Удали их полностью.
...
Исходный ответ Неоны:
{reply}

Правила исправления:
- сначала ответь на смысл последней реплики;
- не придумывай цены, пакеты, рекламу, CRM-интеграцию, e-mail-отправку, 24/7 и другие неподтверждённые услуги;
- если личная задача человека уже ясна, можно мягко предложить короткую встречу с {forms['instrumental']} для показа работающей системы на его примере;
- 1–3 коротких человеческих предложения;
- верни только готовую реплику.
""".strip()
    try:
        corrected = _call_openai(config, instructions, incoming_text)
        if corrected and not _contains_unapproved_offer(corrected):
            return corrected
    except Exception:
        pass
    return _capability_guard_fallback(owner_name, incoming_text, context)
...
def _sanitize_relationship_memory(context):
    ...
    if isinstance(turns, list):
        ...
        for turn in turns:
            ...
            if _contains_unapproved_offer(str(clean_turn.get("neona_reply") or "")):
                clean_turn["neona_reply"] = ""
    ...
    for scalar_key in ("last_incoming", "last_summary"):
        if _looks_like_transcription_artifact(str(mem.get(scalar_key) or "")):
            mem[scalar_key] = ""
    if _contains_unapproved_offer(str(mem.get("last_reply") or "")):
        mem["last_reply"] = ""
...
Когда личная задача уже понятна и человек хочет узнать «что дальше», НЕ уходи в выдуманные тарифы, пакеты, рекламу или коммерческое предложение.
Не проси e-mail для прайса/КП и не продолжай бесконечно «квалифицировать» человека новыми вопросами.
Если его задача уже ясна, это достаточный момент для мостика к встрече: коротко назови, какую реальную часть задачи W уже может снять, и предложи увидеть это на его примере вместе с владельцем аккаунта.
Встреча с владельцем аккаунта — дальняя цель, а не обязательный ответ на каждую реплику.
{meeting_rule}

ГРАНИЦА ПРАВДЫ О ВОЗМОЖНОСТЯХ:
{_AGENCY_W_CAPABILITY_BOUNDARY}
...
def _process_message(
    ...
    # Последняя линия защиты: ни один путь политики не должен отправить
    # выдуманную услугу, цену, пакет или обещание действия, которого система не выполняет.
    if reply:
        reply = _guard_reply_against_unapproved_offers(
            config, owner_name, text, reply, context
        )
